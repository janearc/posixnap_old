package Neopets::NeoMail::NeoFriends;
...
Neopets::NeoMail::NeoFriends - Manage Neo Friends
...
=item $nf = Neopets::NeoMail::NeoFriends -> new;

This constructor takes hash arguments and
returns a neofriends object.  Optional arguments
are:
...
=item $friends = $nf -> list();

Retrieves a list of neo friends and
...
sub list {
  ...
  my $page = $agent -> get( { url => 'http://www.neopets.com/neofriends.phtml' } );
  return [ $page =~ m//g ];
}

=item $nf -> add( $user );

This method sends a friend request
to $user.
