...
Neopets::Config::Text - Colon delimited text file config interface
...
=head1 SYNOPSIS
  ...
  my $file = 'test.txt'; # relative to $ENV{NP_HOME}

  my $c = $config -> read_config( { Text => $file } );
  $config -> write_config( { Text -> $file, contents -> $c } );

=head1 ABSTRACT

This module provides the capability for
colon-delimited text files (see
Neopets::Config::CD_Parse for formats
available formats).  It should never be
used directly, only through its parent
Neopets::Config.

Setting Text => $file in the argument hash
...
sub read_sub_config {
  ...
  die "$0: read_config_file: $docroot/$file does not exist\n"
      unless ( -f "$docroot/$file" );

  my @lines = read_file( "$docroot/$file" );
...
sub write_sub_config {
  ...
  write_file( $docroot."/".$args -> {Text}, $file );
