...
sub market_buy {
	...
	if (($shares_bought =~ /\D/) or (! is_valid_symbol($stock_bought))) {
...
sub market_sell {
	...
	is_valid_symbol($stock_sold) or return UNSPECIFIED_ERROR;
...
sub market_last {
	...
	is_valid_symbol($query) or return UNSPECIFIED_ERROR;
...
# returns whether a symbol is syntactically correct
sub is_valid_symbol {
	my $symbol = shift;
	
	if ($symbol =~ /[^A-Z.]/) {
		utility::debug( "invalid symbol." );
		return 0;
	}
	return 1;
}
