...
# The initialization
    ...
    $nap->command_processor(\&do_stdin_command,\*STDIN);
    $nap->join_channel($channel);
    
# Some startup feedback
    print <<EOF
Dan's correction perlbot.  
stdin commands implemented:
	/last nick
	/whois
	/say
        !pottymouth nick
...
sub do_stdin_command {
   ...
   if ($line =~ /^\/(exit|quit)/) {
       ...
       print "danbot command \"$line\" unknown\n";
       return undef;
   }
   return 1;
}
...
#	$msg =~ m%^\#$chanre ([[:alnum:]]+) \s*(s|tr|y)([/!|:;-])(([^\$\\]|\\.)*?\$?)\3(([^\$\\]|\\.|\$[1-9])*?)(\3([iegdx]*))?\s*$%x ) {
	...
	if ($nick ne $botname) {
	    print "match $op$delimiter$pat$delimiter$repl$delimiter$flags   -   $op / $pat / $repl / $flags\n";
...
"print 'DEBUG $op/$pat/$repl/$flags ', \"\$testmsg\n\"; 
...
\$testmsg;" )) {
			...
			if ((time - $lastre_time) > 10) { 
			    ...
			    print "looping code: $msg $recent_re{$msg}\n";
			}
	...
	#    print "Debug: trying to sanitize $nick\n";
